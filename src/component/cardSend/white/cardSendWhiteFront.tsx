...
class CardSendWhiteFront extends React.Component<cardSendBackProps, any> {
    ...
    cardSendWhiteBack(){
        if (!this.state.toPerson.replace(/\s/g, '').length) {
            alert("받는사람을 입력해주세요.")
        } else if (!this.state.fromPerson.replace(/\s/g, '').length) {
            alert("보내는사람을 입력해주세요.")
        } else {
            this.setState({
                showback : true,
